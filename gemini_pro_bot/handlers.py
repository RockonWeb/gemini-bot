...
async def handle_any_file(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if update.message.photo:
        ...
        file_bytes = await file.download_as_bytearray()  # Changed here
        file_content = BytesIO(file_bytes)
        try:
            file_content = Image.open(file_content)  # Convert to PIL Image
        except Exception as e:
            await init_msg.edit_text(f"Error opening image: {e}")
            return
        ...
        try:
            ...
            if mime_type == 'application/pdf':
                ...
                try:
                    ...
                    decoded_bytes = file_bytes.decode(encoding)
                    ...
                    file_content = highlight(decoded_bytes, lexer, formatter)
                except Exception as e:
                ...
                try:
                    encoding = chardet.detect(file_bytes)['encoding'] or 'utf-8'
                    file_content = file_bytes.decode(encoding)  # Or other appropriate encoding
                except Exception as e:
                    file_content = f"Error decoding text file: {e}"
                ...
                try:
                    ...
                    file_content = f"Error opening image: {e}"

            else:  # For other file types, send the raw bytes
                file_content = bytes(file_bytes)  # Convert to bytes
            ...
            if prompt and isinstance(file_content, str):
                ...
                prompt = f"Отвечай на русском. Проанализируй. {file_content}"
            ...
            if isinstance(file_content, BytesIO): # For image data
                ...
                try:
                    response = await current_model.generate_content_async([prompt or "Отвечай на русском. Проанализируй", file_content], stream=True)
                                ...
                                await init_msg.edit_text(f"Неподдерживаемый тип файла")
                                print('error when file analyzing', {filename})
                                return
