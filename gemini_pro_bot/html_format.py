...
def apply_sub_sup(text: str) -> str:
    """Applies HTML sub and sup tags for chemical formulas, handling existing tags."""

    text = re.sub(r"(?<!<)([a-zA-Z0-9]+)\^([^\s^<]+)", r"\1<sup>\2</sup>", text)
    text = re.sub(r"(?<!<)([a-zA-Z0-9]+)<sub>((?:[^<]|<[^/]|</[^s]|</s[^u]|</su[^b]|</sub[^>])+)</sub>", r"\1<sub>\2</sub>", text)
...
def apply_exclude_code(text: str) -> str:
    ...
    for i, line in enumerate(lines):
        ...
        if not in_code_block:
            ...
            formatted_line = apply_sub_sup(formatted_line)
...
def format_message(text: str) -> str:
    ...
    """
    formatted_text = apply_exclude_code(text) # Убрали escape_html отсюда
